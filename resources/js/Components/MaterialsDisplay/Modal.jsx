...
export const ButtonModal = ({material}) => {
	...
	const isTimeOccupied = ()=>{
		...
		const timeOverLaps = (time1, time2) => {
			...
			const [startTime, endTime] = [momentify(time1[0]), momentify(time1[1])];
			
			// console.log(`is ${startTime} between ${momentify(time2[0])} and ${momentify(time2[1])}  = `+ startTime.isBetween(momentify(time2[0]), momentify(time2[1]), undefined, "[)"));
			
			// console.log(`is ${endTime} between ${time2[0]} and ${time2[1]}  = ` + endTime.isBetween(momentify(time2[0]), momentify(time2[1]), undefined, "(]"));
			
			if( startTime.isBetween(momentify(time2[0]), momentify(time2[1]), undefined, "[)") ||
				endTime.isBetween(momentify(time2[0]), momentify(time2[1]), undefined, "(]") )
			return true;
